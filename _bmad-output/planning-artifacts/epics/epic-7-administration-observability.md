...
**When** the request is processed
**Then** a new `ResumeTemplate` entity is created with `is_prebuilt = true` and `is_published = false` by default; HTTP 201 returned with `TemplateDto`; the endpoint requires `ADMIN` role (`@PreAuthorize("hasRole('ADMIN')")`)
...
**Given** Story 7.3 OTel span propagation is complete and the app is running via `docker compose up`
